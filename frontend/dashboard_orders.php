...
<section class="py-6 md:py-10 space-y-8">
    ...
    <form method="GET" class="glass-morphism rounded-[2rem] p-6 border border-white/10 flex flex-col md:flex-row gap-4 animate__animated animate__fadeInUp">
        <div class="flex-1 min-w-0 overflow-hidden">
            ...
            <input type="date" name="from" value="<?php echo htmlspecialchars($_GET['from'] ?? ''); ?>" class="w-full min-w-0 max-w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#ff610a]/50 transition-all">
        </div>
        <div class="flex-1 min-w-0 overflow-hidden">
            <label class="text-xs uppercase tracking-wider font-black text-gray-500 mb-2 block">To</label>
            <input type="date" name="to" value="<?php echo htmlspecialchars($_GET['to'] ?? ''); ?>" class="w-full min-w-0 max-w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-[#ff610a]/50 transition-all">
        </div>
